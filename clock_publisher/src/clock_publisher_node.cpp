...
public:
  ...
  : Node("clock_publisher"), clock(std::make_shared<rclcpp::Clock>(RCL_STEADY_TIME))
  {
    ...
    declare_parameter<double>("time_resolution_ms", 5.0);
    time_resolution_ms = get_parameter("time_resolution_ms").as_double();

    start_time = clock->now();
    publisher = create_publisher<rosgraph_msgs::msg::Clock>("/clock", 1);
    using std::chrono_literals::operator""ms;
    timer = rclcpp::create_timer(this, clock, time_resolution_ms * 1ms, [this]() {
